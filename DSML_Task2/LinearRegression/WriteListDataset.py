...
student = pd.read_json("C:/Users/clara/PycharmProjects/prog2/DSML_Task2/FileGenerated/ListDataset.txt", orient='records',dtype=True)
...
for i in range(0,len(matr)):
        listStudentTemp = [[int(matr[i]), int(cf[i]), int(targetValue[i]), int(secondo[i]), int(terzo[i]), int(tot[i]), int(CDS[i]), int(Tipo_Cds[i]), int(coorte[i]), int(anni_carriera[i]), int(anno_diploma[i]),int(voto_diploma[i]), int(Cod_School[i]), int(tipo_mat[i]), int(anno_laurea[i]), int(voto_laurea[i]), int(erasmus[i]), int(tesi_estero[i]), int(mot_sta_stud[i]), int(sta_stud[i]), int(fc[i]), int(classe[i])]]
        ListStudentModify.append(listStudentTemp)


print(len(ListStudentModify))
...
with open('ListaStudentiModificata.txt', 'w') as file:
    file.write(json.dumps(ListStudentModify))
