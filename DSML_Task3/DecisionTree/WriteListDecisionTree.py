...
student = pd.read_json("C:/Users/salva/Desktop/Università/Magistrale - Data Science/Data Science and Machine Learning/MachineLearning-StudentActivity/DSML_Task3/Dataset/ListDataset.txt", orient='records',dtype=True)
...
for i in range(0,len(matr)):
    if sta_stud[i] == degree:
        predictiveAttributeTemp = [int(matr[i]), int(cf[i]), int(targetValue[i]), int(secondo[i]), int(terzo[i]), int(tot[i]), int(CDS[i]), int(Tipo_Cds[i]), int(coorte[i]), int(anni_carriera[i]), int(anno_diploma[i]),
                        int(voto_diploma[i]), int(Cod_School[i]), int(anno_laurea[i]), int(voto_laurea[i]), int(erasmus[i]), int(tesi_estero[i]), int(mot_sta_stud[i]), int(sta_stud[i]), int(fc[i])]
...
with open('filenuovi/matr.txt', 'w') as file:
    file.write(json.dumps(matr_dict))

with open('filenuovi/cf.txt', 'w') as file:
    file.write(json.dumps(cf_dict))

with open('filenuovi/CDS.txt', 'w') as file:
    file.write(json.dumps(CDS_dict))

with open('filenuovi/Cod_school.txt', 'w') as file:
    file.write(json.dumps(Cod_School_dict))

with open('filenuovi/Mot_sta_stud.txt', 'w') as file:
    file.write(json.dumps(mot_sta_stud_dict))

with open('filenuovi/sta_stud.txt', 'w') as file:
    file.write(json.dumps(sta_stud_dict))

with open('filenuovi/Tipo_CDS.txt', 'w') as file:
    file.write(json.dumps(Tipo_Cds_dict))

with open('filenuovi/predictiveDegree.txt', 'w') as file:
    file.write(json.dumps(predictiveAttribute))

with open('filenuovi/predictiveNotDegree.txt', 'w') as file:
