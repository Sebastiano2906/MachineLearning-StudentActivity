...
#
#
# predictiveAttributeDegree = pd.read_json("C:/Users/salva/Desktop/Università/Magistrale - Data Science/Data Science and Machine Learning/MachineLearning-StudentActivity/DSML_Task3/DecisionTree/filenuovi/predictiveDegree.txt", orient='records', dtype=True,typ="series")
# predictiveAttributeNotDegree = pd.read_json("C:/Users/salva/Desktop/Università/Magistrale - Data Science/Data Science and Machine Learning/MachineLearning-StudentActivity/DSML_Task3/DecisionTree/filenuovi/predictiveNotDegree.txt", orient='records', dtype=True,typ="series")
#
#
# train_set = []
# test_set = []
...
#         count = count + 1
#         train_set.append([predictiveAttributeDegree[i][11], predictiveAttributeDegree[i][12], predictiveAttributeDegree[i][2]])
#         train_result.append([predictiveAttributeDegree[i][19]])
#     else:
#         test_set.append([predictiveAttributeDegree[i][11], predictiveAttributeDegree[i][12], predictiveAttributeDegree[i][2]])
#         test_result.append([predictiveAttributeDegree[i][19]])
# train_percent = (len(predictiveAttributeNotDegree)/100)*80
# count = 0
# for i in range(len(predictiveAttributeNotDegree)):
#     if count < train_percent:
#         count = count + 1
#         train_set.append([predictiveAttributeNotDegree[i][11], predictiveAttributeNotDegree[i][12],predictiveAttributeNotDegree[i][2]])
#         train_result.append([predictiveAttributeNotDegree[i][19]])
#     else:
#         test_set.append([predictiveAttributeNotDegree[i][11], predictiveAttributeNotDegree[i][12], predictiveAttributeNotDegree[i][2]])
#         test_result.append([predictiveAttributeNotDegree[i][19]])
#
#
...
for i in range(len(predictiveAttributeDegree)):
    if count < train_percent:
        count = count + 1
        train_set.append([predictiveAttributeDegree[i][0], predictiveAttributeDegree[i][1],
                          predictiveAttributeDegree[i][6], predictiveAttributeDegree[i][7], predictiveAttributeDegree[i][8],
                          predictiveAttributeDegree[i][9], predictiveAttributeDegree[i][10],predictiveAttributeDegree[i][11],
                          predictiveAttributeDegree[i][12], predictiveAttributeDegree[i][13], predictiveAttributeDegree[i][14],
                          predictiveAttributeDegree[i][15], predictiveAttributeDegree[i][16], predictiveAttributeDegree[i][17],
                          predictiveAttributeDegree[i][18]])

        train_result.append([predictiveAttributeDegree[i][19]])
    else:
        test_set.append([predictiveAttributeDegree[i][0], predictiveAttributeDegree[i][1],
                         predictiveAttributeDegree[i][6], predictiveAttributeDegree[i][7], predictiveAttributeDegree[i][8],
                         predictiveAttributeDegree[i][9], predictiveAttributeDegree[i][10],predictiveAttributeDegree[i][11],
                         predictiveAttributeDegree[i][12], predictiveAttributeDegree[i][13], predictiveAttributeDegree[i][14],
                         predictiveAttributeDegree[i][15], predictiveAttributeDegree[i][16], predictiveAttributeDegree[i][17],
...
for i in range(len(predictiveAttributeNotDegree)):
    if count < train_percent:
        count = count + 1
        train_set.append([predictiveAttributeNotDegree[i][0], predictiveAttributeNotDegree[i][1],
                          predictiveAttributeNotDegree[i][6], predictiveAttributeNotDegree[i][7], predictiveAttributeNotDegree[i][8],
                          predictiveAttributeNotDegree[i][9], predictiveAttributeNotDegree[i][10],predictiveAttributeNotDegree[i][11],
                          predictiveAttributeNotDegree[i][12], predictiveAttributeNotDegree[i][13], predictiveAttributeNotDegree[i][14],
                          predictiveAttributeNotDegree[i][15], predictiveAttributeNotDegree[i][16], predictiveAttributeNotDegree[i][17],
                          predictiveAttributeNotDegree[i][18]])

        train_result.append([predictiveAttributeNotDegree[i][19]])
    else:
        test_set.append([predictiveAttributeNotDegree[i][0], predictiveAttributeNotDegree[i][1],
                         predictiveAttributeNotDegree[i][6], predictiveAttributeNotDegree[i][7], predictiveAttributeNotDegree[i][8],
                         predictiveAttributeNotDegree[i][9], predictiveAttributeNotDegree[i][10],predictiveAttributeNotDegree[i][11],
                         predictiveAttributeNotDegree[i][12], predictiveAttributeNotDegree[i][13], predictiveAttributeNotDegree[i][14],
                         predictiveAttributeNotDegree[i][15], predictiveAttributeNotDegree[i][16], predictiveAttributeNotDegree[i][17],
...
print(regressor.score(test_set, test_result))
#              matr cf    2  3 tot cds tipoCds coorte annicarriera annodiploma votodip codschool tipoMat annolaur votolaur erasmus tesi mot_sta sta fc
newStudent = [[2162, 2158, 1, 1, 2014, 3, 2013, 87, 133, 2015, 101, 0, 0, 3, 7]]
real_value = [0]
predicted = regressor.predict(newStudent)
print("Predicted: ", predicted)
print("MSE: ", mean_squared_error(real_value, regressor.predict(newStudent)))
